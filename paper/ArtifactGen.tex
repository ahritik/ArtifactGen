...
We adopt a denoising diffusion probabilistic model (DDPM) \citep{ho2020denoisingprobabilistic} with a 1D U\!-Net backbone. Inputs $x\in\mathbb{R}^{C\times T}$ are standardized per recording/channel (z-score). Timestep embeddings (sinusoidal) and label embeddings are fused and injected via FiLM layers to modulate intermediate activations; we reserve a null label to support classifier-free guidance during sampling \citep{ho2022classifierfree}. The network predicts additive noise with an MSE loss.
...
\paragraph{Signal fidelity.} (Detailed numerical table to be inserted; current pipeline auto-generates `table_bandpower.tex` with corrected band deltas including $\gamma$.)

\paragraph{Distributional two-sample tests.} We observe 1-NN accuracies of X (WGAN) vs Y (DDPM) (placeholder values to update) in embedding space, and C2ST logistic accuracies of A vs B, confirming non-trivial but reduced separability relative to the procedural baseline (Z). Bootstrap MMD CIs (95\%) show non-overlap between procedural and learned models, indicating meaningful gains beyond trivial surrogates.
