...
def train_wgan(cfg, device: torch.device):
    """
    Trains a Wasserstein GAN with Gradient Penalty (WGAN-GP) for EEG artifact generation.

    This function implements the WGAN-GP algorithm to train a generator and critic network
    on EEG windows. The generator learns to produce synthetic EEG artifacts, while the critic
    distinguishes real from fake samples. Gradient penalty ensures stable training.

    Args:
        cfg: Configuration dictionary containing training parameters.
        device: PyTorch device (CPU or GPU) for computations.
    """
